...
class AdminControllerTest {
    ...
    @Test
    @DisplayName("Изменение роли пользователя - роль null")
    ...
    @Test
    @DisplayName("Изменение роли пользователя - неверное значение роли")
    void updateUserRole_InvalidRoleValue() throws Exception {
        String invalidJson = """
        {
            "role": "SUPERUSER"
        }
    ...
    void updateCardStatus_Success() throws Exception {
        ...
        when(adminService.updateCardStatus(1L, CardStatus.BLOCKED)).thenReturn(card);
        ...
        mockMvc.perform(put("/admin/cards/1/status")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.status", is(CardStatus.BLOCKED.name())));
    }
    ...
    void updateCardStatus_CardNotFound() throws Exception {
        when(adminService.updateCardStatus(999L, CardStatus.BLOCKED))
    ...
    @Test
    @DisplayName("Изменение статуса карты - нельзя установить EXPIRED вручную")
    void updateCardStatus_CannotSetExpiredManually() throws Exception {
        when(adminService.updateCardStatus(1L, CardStatus.EXPIRED))
                .thenThrow(new IllegalArgumentException("Cannot manually set EXPIRED status"));

        mockMvc.perform(put("/admin/cards/1/status")
                        .param("status", CardStatus.EXPIRED.name())
                        .with(user(adminDetails)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message", containsString("Cannot manually set EXPIRED")));
    }
